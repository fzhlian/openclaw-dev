...
- `snippetZh` / `summaryZh`：可选但推荐；当前最终输出仅支持中文，因此优先使用这些字段
- 当前渲染脚本不会在摘要阶段强制翻译原始摘要；若没有中文摘要字段，会退回 `snippet` / `summary`
...
- 若剔除缺链接条目后已不足以形成有效摘要，不输出正常 `文章清单`，改走“检索参数 + 已发现结果 + 局限与建议”
- 若要使用“按主题分组+逐条”，则每条结果都应提供 `topic` / `queryTopic` / `keyword` / `query` 之一；否则不应进入分组渲染
