...
- 若输入时间范围为 `近1周 / 近2周 / 近1个月`，或 `近一周 / 近两周 / 近一个月` 这类更短的中文表达，也应归一化为同一标准格式再回显
- 若输入时间范围为 `1月 / 一月 / 最近一月 / 过去一月 / 近一月` 这类“月”的自然表达，也应归一化为 `最近 30 天`
- 若输入时间范围为 `今天 / 今日 / 昨天 / 昨日` 这类相对日表达，也应归一化为 `最近 1 天`
- 若输入时间范围为 `本周 / 本月` 这类当前周期表达，也应分别归一化为 `最近 7 天 / 最近 30 天`
- 若输入时间范围为 `这周 / 这星期 / 这个月 / 这月` 这类当前口语表达，也应分别归一化为 `最近 7 天 / 最近 30 天`
- 若输入时间范围为 `上周 / 上星期 / 上个月 / 上月` 这类上一周期表达，也应分别归一化为 `最近 7 天 / 最近 30 天`
- 若输入时间范围为 `本星期 / 这一周 / 这一月 / 上一周 / 上一星期 / 上一月 / 上一个月` 这类相邻口语表达，也应分别归一化为 `最近 7 天 / 最近 30 天`
- 若输入时间范围为 `本礼拜 / 这礼拜 / 上礼拜` 这类更口语化的周表达，也应归一化为 `最近 7 天`
- 若输入时间范围为 `过去7天 / 过去24小时` 这类常见中文表达，也应归一化为同一标准格式再回显
- 若输入时间范围为 `最近一周 / 过去一周 / 最近一个月 / 过去一个月` 这类口语表达，也应归一化为同一标准格式再回显
- 若输入时间范围为 `最近1周 / 过去1周 / 最近1个月 / 过去1个月` 这类数字化中文表达，也应归一化为同一标准格式再回显
- 若输入时间范围直接写成 `一天 / 一周 / 两周 / 一个月`，也应归一化为 `最近 1 天 / 最近 7 天 / 最近 14 天 / 最近 30 天`
- 若输入时间范围直接写成 `1周 / 2周 / 1个月`，也应归一化为 `最近 7 天 / 最近 14 天 / 最近 30 天`
...
- 若时间范围、频率或输出模式仅多了句末标点，如 `最近7天。`、`执行一次。`、`总览+逐条。`，也应先去掉尾部标点再归一化
- 若时间范围、频率、输出模式或输出语言被引号或括号包裹，如 `“最近7天”`、`（执行一次）`、`《总览+逐条》`、`“中文”`，也应先去掉外层包裹符号，再做标准化与校验
- 若输出语言仅多了句末标点，如 `中文。`，也应先去掉尾部标点，再按当前仅支持中文的规则校验
- 若输入关键词或网站使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|`、全角斜杠 `／`，或任一侧带空格的 ASCII 斜杠（如 ` / `、` /foo`、`foo/ bar`），也应按普通逗号拆分，而不是把整串当成单个值
- 若输入关键词夹带句末标点，如 `OpenAI。`、`Gemini！`，也应先去掉尾部标点，再进入确认与查询生成
- 若输入关键词被引号包裹，如 `“OpenAI”`，也应先去掉外层引号，再进入确认与查询生成
...
- 若输入网站夹带句末标点，如 `openai.com.`、`blog.google。`，也应先去掉尾部标点，再归一化为主域名
- 若输入网站被括号或引号包裹，如 `（openai.com）`、`“openai.com”`，也应先去掉外层包裹符号，再归一化为主域名
...
- 不自动扩词
- 不自动英文化
- 若关键词或站点使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|`、全角斜杠 `／`，或任一侧带空格的 ASCII 斜杠（如 ` / `、` /foo`、`foo/ bar`），仍能正确拆分成多项查询
- 若同一关键词仅大小写不同地重复出现，如 `OpenAI, openai`，不应生成重复查询
- 若排除词仅大小写不同地重复出现，如 `ads,Ads`，也不应重复拼进同一条查询
- 若排除词夹带句末标点，如 `ads。`、`tracking！`，也应先去掉尾部标点，再拼进查询
- 若排除词被引号包裹，如 `“ads”`，也应先去掉外层引号，再拼进查询
...
- 不在目标域名内的结果会被丢弃
- 若 `--site` 使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|`、全角斜杠 `／`，或任一侧带空格的 ASCII 斜杠（如 ` / `、` /foo`、`foo/ bar`）分隔多个域名，仍能正确拆分并过滤
