...
- 若输入时间范围为 `24h / 1d / 7d / 14d / 30d`，或 `24 小时 / 1 天 / 7 天 / 14 天 / 30 天` 等简写，应先归一化为 `最近 24 小时 / 最近 1 天 / 最近 7 天 / 最近 14 天 / 最近 30 天` 再回显
...
- `## 检索参数` 中显示本次 `频率`
- `## 检索参数` 中仍显示 `输出语言：中文`
- 若渲染输入里传入 `24h / 7d / 30d`，或 `24 小时 / 7 天 / 30 天` 等时间范围简写，最终 `## 检索参数` 中也应回显标准中文写法
