...
- 若输入关键词或网站使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|`，或带空格斜杠 ` / `，也应按普通逗号拆分，而不是把整串当成单个值
...
- 不自动扩词
- 不自动英文化
- 若关键词或站点使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|` 或带空格斜杠 ` / `，仍能正确拆分成多项查询
...
- 不在目标域名内的结果会被丢弃
- 若 `--site` 使用全角逗号 `，`、中文顿号 `、`、中文分号 `；`、英文分号 `;`、竖线 `|` 或带空格斜杠 ` / ` 分隔多个域名，仍能正确拆分并过滤
