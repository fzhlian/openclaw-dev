...
- 若 Telegram 消息主体只是一个或多个文章 URL，或同时出现“帮我收录 / 立即分析 / 晚上统一发给我”这类指令，应优先触发 `skills/article-digest/`，而不是把链接写入临时文件或伪造工具调用
- 对这类文章收录请求，默认入口是可直接执行的 `skills/article-digest/scripts/ingest_message.py "<原始消息>"`；只有用户明确在做仓库开发任务时，才继续走普通开发流
- 处理文章收录消息时，不要生成 `<function/...>` 之类伪标签，也不要把 URL 先落到 `link.txt`、`tmp.txt` 之类中间文件再说
